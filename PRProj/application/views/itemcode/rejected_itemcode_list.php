...
<li class="active" style="color:#FFFFFF;">Reject ItemCode</li>
 ...
 <br><br><table id="example" class="table table">
                ...
                <tbody>
				 ...
				 $itemc_approved = $this->method_call->rejected_itemcodelist($emp_code);
...
foreach ($itemc_approved->result() as $row)  
         {  ?>
            ...
            <td >  <a href="<?php echo site_url('purchase/itemcode/itemcode_draft/'.$row->iten_gen_id);?>"> ItemCode <?php echo $row->iten_gen_id;?> </a></td> 
			...
			$status="";
			if($comp == "Reject_by_taxation_dept"){ $status="Level1:  Rejected By Taxtation Dept";}
			if($comp =="Reject_by_sap_dept"){$status="Level2: Rejected By SAP Dept";}
			echo $status;
				?>
			</td>
		 ...
		 <!-- rejected by SAP dept -->
		 
		 <?php
				
				
				 $itemc_approved = $this->method_call->rejected_Sap_list($emp_code);
 if($itemc_approved!=null){
