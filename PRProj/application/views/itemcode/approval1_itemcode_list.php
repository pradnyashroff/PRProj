...
<li class="active" style="color:#FFFFFF;">ItemCode</li>
 ...
 <br><br><table id="example" class="table table">
		...
		<!-- approval1 taxtation dept -->
					<?php
				
				
				 $itemc_approved = $this->method_call->approval_itemcodelist($emp_code);
...
foreach ($itemc_approved->result() as $row)  
         {  ?>
            ...
            <td >  <a href="<?php echo site_url('purchase/itemcode/approve_taxtation/'.$row->iten_gen_id);?>"> ItemCode <?php echo $row->iten_gen_id;?> </a></td> 
			<td><?php echo $row->qcs_id;?></td>
			<td><?php echo $row->plant_code;?></td>  				 
            <td><?php echo $row->item_gen_date;?></td>
			<td>
			<?php 
			$comp=$row->item_gen_status;
			$status = "";
			if($comp == "Submited_taxtation_dept")
			{
				$status="Waiting for HSN Code Updation";
			}
			echo $status 
			?>
			</td>
		 ...
		 <!-- Approval2 list SAP dept -->
		 <?php
				
				
				 $itemc_approved_app2 = $this->method_call->sap_approval_itemcodelist($emp_code);
 if($itemc_approved_app2!=null){
	 	   
	$sr_no=1;			  
foreach ($itemc_approved_app2->result() as $row)  
         {  ?>
			
			
			<tr>  
            <td style="display:none;"><?php echo $row->iten_gen_id;?></td>  
            <td >  <a href="<?php echo site_url('purchase/itemcode/approve_by_sap/'.$row->iten_gen_id);?>"> ItemCode <?php echo $row->iten_gen_id;?> </a></td> 
			<td><?php echo $row->qcs_id;?></td>
			<td><?php echo $row->plant_code;?></td>  				 
            <td><?php echo $row->item_gen_date;?></td>
			<td>
			...
			$comp=$row->item_gen_status;
			$status = "";
			if($comp == "Approve_by_taxation_dept")
			{
				$status="Waiting for ItemCode Creation";
			}
			echo $status 
			?>
			</td>
