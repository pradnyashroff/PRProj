...
<html>
    ...
    <body class="hold-transition skin-blue sidebar-mini" style="font-family:'exo';" >   	  
        ...
        <div class="content-wrapper"  style="background-color:#3482AE;text-transform: uppercase;">
            ...
            <section class="content-header">      
               <h1 style="color:#FFFFFF; font-family:'exo';text-transform: uppercase;">LOCAL CONVEYANCES ACCEPTED CLAIM TABLE
                    <?php
                    if (isset($error)) {
	...
	<div class="container">
    <form method="post" id="add_localRim_detailsEdit" name="add_localRim_detailsEdit" action="<?php echo site_url('ELAR/local_reim/Local_cntr/showSancAuthoReportAccept') ?>" >
    <div class="row">
        <div class='col-sm-4'>
...
echo '<tr>' . '<td class="e_id"> <a href="'. site_url('ELAR/local_reim/Local_cntr/sanc_autho_local_claim_accpt_view/'.$value->lrcm_id).'" class="glyphicon glyphicon-edit" style="color:red;">' . $value->lrcm_id . '</a></td>' . '<td>' . $value->reg_date . '</td>' . '<td>' . $value->emp_name . '</td>' . '<td>' . $value->emp_code . '</td>'.'<td>' . $value->dept_name . '</td>'.'<td>' . $value->plant_name . '</td>'.'<td>' . $value->total_amt . '</td>' .''. '</td> <td>You Approved</td>' . '<tr/>';
}
