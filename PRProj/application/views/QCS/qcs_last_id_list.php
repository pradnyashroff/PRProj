...
<li class="active" style="color:#FFFFFF;text-transform: uppercase;">QCS Step2 Pending List</li>
 ...
 <br><br><table id="example" class="table table">
               ...
               <tr style="background-color:#3482AE;color:#FFFFFF;">
				...
				<?php
				 ...
				 $qcs_lastid= $this->method_call->qcs_step2list_lastrecord($emp_code,$pr_id);
 if($qcs_lastid!=null){
	 	   
	$sr_no=1;			  
foreach ($qcs_lastid->result() as $row)  
         {  ?>
			
			
			<tr>  
            <td style="display:none;"><?php echo $row->qcs_id;?></td>  
            <td >  <a href="<?php echo site_url('purchase/QCS/qcs_step_two/'.$row->qcs_id);?>"> QCS <?php echo $row->qcs_id;?> </a></td> 
