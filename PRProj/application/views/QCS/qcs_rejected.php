...
<li class="active" style="color:#FFFFFF;text-transform: uppercase;">Rejected QCS</li>
 ...
 <br><br><table id="example" class="table table">
               ...
               <tr style="background-color:#3482AE;color:#FFFFFF;">
				...
				<?php
				 ...
				 $qcs_reject= $this->method_call->qcs_rejected_list($emp_code);
 if($qcs_reject!=null){
	 	   
	$sr_no=1;			  
foreach ($qcs_reject->result() as $row)  
         {  ?>
		   ...
		   <?php
			$comp=$row->qcs_status;
$status="";
if($comp == "Sourcing_Mgr_reject"){ $status="Level1:  Rejected By Sourcing Manager";}
if($comp == "Sourcing_Head_Reject"){ $status="Level2:  Rejected By Sourcing Head"; }
if($comp == "CFO_Reject"){ $status="Level3:  Rejected By CFO"; }
if($comp == "COO_Reject"){ $status="Level4:  Rejected By COO"; }
if($comp == "QCS_Reject") { $status="Level5:  Rejected By MD Office"; }
			echo $status 
			?>
