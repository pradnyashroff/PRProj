...
<div class="form-group col-sm-12">
            ...
            <?php echo $row4->ftgs_sup1_nm; ?>
						...
						<?php 
						 ...
						 <form method="post" id="pend_pr" name="pend_pr" action="<?php echo site_url('FTGS_PR/Ftgs_pr/QcsApproval3_insert') ?>" >
							...
							<?php
								 $autho_codedt=$this->method_call->getApproval4AuthoDetails();
								 if($autho_codedt!=null)
						{
							foreach ($autho_codedt->result() as $rowauth)  
							{   
							?>
						
                                  <input type="hidden" value="<?php echo $rowauth ->auth_id; ?>" name="txt_approval4_Autho" class="form-control"  required>	
...
?>	
								...
								<input type="hidden" name="txt_QCSUseremail" value="<?php print_r($owner_eid['emp_email']); ?>" readonly  class="form-control"  required>
									...
									<?php
								$ftgsActionData = $this->method_call->ftgsApproval3Action($emp_code,$ftgs_qcs_id);
...
<?php 
								...
								else if($emp_code == '100098'){
										echo ' in level 9';
									?>
									<form method="post" id="pend_pr" name="pend_pr" action="<?php echo site_url('FTGS_PR/Ftgs_pr/QcsApproval5_insert') ?>" >
											<div class="form-group col-lg-12">
										<label class="col-lg-4 pull-left control-label">9. &nbsp;&nbsp;&nbsp;Action</label>
										<div class="input-group  col-lg-6">
											<input type="radio" name="ftgs_status" value="1" required > &nbsp;&nbsp;APPROVE</input>
									...
									<?php
								$ftgsActionData = $this->method_call->ftgsApproval5Action($emp_code,$ftgs_qcs_id);
                        ...
                        ?>
								...
								else
								{
									...
									/* 100198 login code approval level 8*/
									?>	
										<form method="post" id="pend_pr" name="pend_pr" action="<?php echo site_url('FTGS_PR/Ftgs_pr/QcsApproval4_insert') ?>" >
									<div class="form-group col-lg-12">
										<label class="col-lg-4 pull-left control-label">9. &nbsp;&nbsp;&nbsp;Action</label>
										<div class="input-group  col-lg-6">
											<input type="radio" name="ftgs_status" value="1" required > &nbsp;&nbsp;APPROVE</input>
									...
									<?php
								$ftgsActionData = $this->method_call->ftgsApproval4Action($emp_code,$ftgs_qcs_id);
                        ...
                        ?>
							...
							<?php
										...
										if($row4->ftgs_outside_budget == 'YES' || $row4->ftgs_non_properitery_item == 'YES' ||$row4->ftgs_properitery_item == 'YES' ||$row4->ftgs_post_grn_nonproprietary == 'YES'
										|| $row4->ftgs_post_grn_proprietary == 'YES' || $row4->ftgs_delivery_terms == 'YES' || $row4->ftgs_cost_reimb_cust == 'YES')
										{
											?>
						<?php
													 $autho_codedt=$this->method_call->getApproval5AuthoDetails();
													 if($autho_codedt!=null)
												{
												foreach ($autho_codedt->result() as $rowauth)  
												{   
