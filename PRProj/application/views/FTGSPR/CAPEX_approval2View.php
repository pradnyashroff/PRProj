...
<li class="active" style="color:#FFFFFF;">View CAPEX
    ...
    <!-- Main content -->
	  ...
	  <form method="post" role="form" enctype="multipart/form-data"  action="<?php echo site_url('FTGS_PR/Ftgs_pr/insert_capexApproval2') ?>" >  
 <!-- Content Wrapper. Contains page content -->
 <div class="wrapper">
...
<!-- Asset code -->
 ...
 <div class="box-header with-border">
					   ...
					   <div class="box-header with-border">
              <h3 class="box-title">4.  Asset Code Creation &nbsp;&nbsp;&nbsp;&nbsp; &nbsp;&nbsp;   <img src="<?php echo base_url(); ?>assets/images/excel.jpg" style="heu=ight:30px;width:30px;" onclick="pradnya(<?php echo $qcs_row->ftgs_capex_owner_code;?>)"  ></h3>
				<button type="button"  id="btnExport" name="btnExport" onclick="pradnya(<?php echo $qcs_row->ftgs_capex_owner_code;?>)" class="btn" style="display: none;">Basic</button>
		...
		<tbody>
							...
							<?php $view_item = $this->method_call->qcs_items_assetcode($ftgs_capex_id);
							$proj_det= $this->method_call->pr_proj_detail($ftgs_capex_id); 
...
foreach ($view_item->result() as $row_asset1)  
         ...
         { 
			if($row_asset1->ftgs_q_uom =="Each"){		
	
		
					for($i=0;$i < $row_asset1->ftgs_q_req_quantity; $i++){
			...
			//echo "test assset id------<br>".$row_asset1->asset_id;
				...
				<tr> 
			<td><?php echo $row_asset1->ftgs_asset_id; ?></td>
			<td><?php echo $row_asset1->ftgs_cap_unit; ?></td>
			<td><?php echo $qcs_row->ftgs_capex_ion_no; ?></td>
			<td><?php echo $qcs_row->ftgs_fund_no; ?></td>
			<td><?php print_r ($proj_det['ftgs_project_detail']);?></td>
			<td><?php echo $qcs_row->ftgs_proj_comp_date; ?></td>
		
			<td><?php echo $row_asset1->ftgs_q_item_code; ?></td> 
			<td><?php echo $row_asset1->ftgs_q_item_description; ?></td>  
			<td><?php echo $row_asset1->ftgs_q_uom; ?></td> 
            <td><?php echo $row_asset1->ftgs_q_req_quantity; ?></td>  
			<td><?php echo $row_asset1->ftgs_cost_center; ?></td>  
			<td><?php echo $row_asset1->ftgs_asset_grp; ?>  </td>  
			<td></td>
			<td></td>
			<td></td>
			</tr>
		...
		elseif($row_asset1->ftgs_q_uom !="Each") {
...
<?php
								$ftgsActionData = $this->method_call->getCaxapp3AuthoDetails();
                        ...
                        ?>
									...
									<?php
								$ftgsActionData = $this->method_call->ftgscpx2IDAction($emp_code,$ftgs_capex_id);
