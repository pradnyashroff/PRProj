...
<head>
  ...
  <title>View Pendding Interplant PR FTGS </title>
						...
						<div class="form-group col-sm-6">
								...
								<div class="form-group col-sm-12">
								 <table id="example" class="table table-bordered table-striped" style="font-size: 12px!important;">
									...
									<div class="col-xs-12">
									 <label class="col-sm-4 pull-left control-label">18 &nbsp;&nbsp;&nbsp;&nbsp;Asset Code Creation</label>
									 <label class="col-sm-8 pull-left control-label"><img src="<?php echo base_url(); ?>assets/images/excel.jpg" style="heu=ight:30px;width:30px;" onclick="AssetCode(<?php echo $row->ftgs_pr_owner_code;?>)"  ></label>
										<div class="box-body " hidden>
											<div class="box-body table-responsive">
												<table id="exampletestExport" name="exampletestExport"  class="table table">
													<thead>
														<tr>
															<th>ITEM CODE</th>
															<th>ITEM DESCRIPTION</th>
													...
													<tbody>
														<?php $item= $this->method_call->ftgs_item_list($ftgs_pr_id);
																	if($item!=null){
																	$sr_no=1;			  
																	foreach ($item->result() as $row3)  
																	{
																		if($row3->ftgs_uom =="Each")
																		{		
																			for($i=0;$i < $row3->ftgs_req_qty; $i++)
																				{																
														?>
														<tr>
															<td><?php echo $row3->ftgs_item_id; ?></td>
															...
															<td><?php echo $row3->ftgs_cust_code; ?></td>
															<td><?php echo $row3->ftgs_sales_material; ?></td>
															<td><?php echo $row3->ftgs_ecn_new; ?></td>
															<td><?php echo $row3->ftgs_ecn_newcode; ?></td>
														</tr>
									 ...
									 <form method="post" id="pend_pr" name="pend_pr" action="<?php echo site_url('FTGS_PR/Ftgs_pr/ASSAuth_insert') ?>"  enctype='multipart/form-data'>
									
									<div class="form-group col-lg-12">
										<label class="col-lg-4 pull-left control-label">19. &nbsp;&nbsp;&nbsp;Asset code Attachment</label>
										<div class="input-group  col-lg-6">
											 <input type ="file" class ="form-control" required name ="assetFile_attach_ftgs"/>
										</div>
									</div>
									<div class="form-group col-lg-12">
										<label class="col-lg-4 pull-left control-label">20. &nbsp;&nbsp;&nbsp;ION NO.</label>
										<div class="input-group  col-lg-6">
											 <input type="text" name="ionno" value="<?php echo $row->ftgs_ion_no; ?>" style="background-color:#E6F2FF;" readonly class="form-control"  required>
										</div>
									</div>
									<div class="form-group col-lg-12">
										<label class="col-lg-4 pull-left control-label">21. &nbsp;&nbsp;&nbsp;Action</label>
										<div class="input-group  col-lg-6">
											<input type="radio" name="ftgs_status" value="1" required > &nbsp;&nbsp;APPROVE</input>
												&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;
											<input type="radio" name="ftgs_status" required value="2" >&nbsp;&nbsp; REJECT</input>
										</div>
									</div>
									<div class="form-group col-lg-12">
										<label class="col-lg-4 pull-left control-label">22. &nbsp;&nbsp;&nbsp;Approver Comment</label>
										<div class="input-group  col-lg-6">
									...
									<?php $plant = $row->ftgs_plant_code;
										$Asset_code=$this->method_call->getAssetAuthoDetails();
										$ACCCode=$Asset_code['Asset_code'];
									?>
										<input type="hidden" value="<?php print_r($Asset_code['Asset_code']); ?>" name="txt_Asset_Autho" class="form-control"  required>	
									...
									<?php $AssetCodeAuthoEmail=$this->method_call->fetchAssetMailDetails($ACCCode);?>
									...
									<input class="form-control" type="hidden" name="txtIONAuthoEmail" value="<?php print_r($AssetCodeAuthoEmail['AssetEmail']); ?>"/>
								    ...
								    <input class="form-control" type="hidden" name="txtTotalAmt" value=" <?php echo $final_rate; ?>"/>
									 ...
									 <input type="hidden" name="txt_owner_code" placeholder="" value="<?php echo $row->ftgs_pr_owner_code; ?>" readonly class="form-control"  required>
								...
								<?php $autho_code=$this->method_call->getItemCodeAuthoDetails($plant);?>
									<input type="hidden" value="<?php print_r($autho_code['autho_code']); ?>" name="txt_ItemCode_Autho" class="form-control"  required>	
								<?php $ION_code=$this->method_call->getIONAuthoDetails($plant);?>
									<input type="hidden" value="<?php print_r($ION_code['ION_Auth']); ?>" name="txt_ION_Autho" class="form-control"  required>	
								<?php $Asset_code=$this->method_call->getAssetAuthoDetails();?>
									<input type="hidden" value="<?php print_r($Asset_code['Asset_code']); ?>" name="txt_Asset_Autho" class="form-control"  required>	
								<?php $ftgsActionData = $this->method_call->ftgsACCAuthoActionData($emp_code,$ftgs_pr_id);
	...
	function exportMe(owner){
		...
		var FTGS_owner = owner; 
		
		alert(FTGS_owner); 
		var ftgs_id = "<?php echo $ftgs_pr_id ?>"; 
        var dt = new Date();
        var day = dt.getDate();
        ...
        a.download = FTGS_owner+"-FTGSID-"+ ftgs_id +"-"+ postfix + '.xls';
