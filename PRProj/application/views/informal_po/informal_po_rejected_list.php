...
<head>
  ...
  <title>Informal PO Rejected List </title>
...
<li class="active"style="color:#FFFFFF;text-transform: uppercase;">Informal PO Rejected List</li>
 ...
 <br><br><table id="example" class="table">
			...
			<!-- rejected  list -->
				 ...
				 $rejected_po = $this->method_call->informal_po_rejectlist($emp_code);
 if($rejected_po!=null){
	 	   
	$sr_no=1;			  
foreach ($rejected_po->result() as $row)  
         {  ?>
		<?php $po_owner_nm= $this->method_call->fetch_emp_name($row->informal_po_empcode); ?>
            ...
            <td >  <a href="<?php echo site_url('purchase/informal_po/reject_view/'.$row->informal_po_id);?>"> Informal PO No <?php echo $row->informal_po_id;?> </a></td> 
			<td><?php echo $row->informal_po_date;?></td>
							 
            <td><?php echo $po_owner_nm;?></td>
			<td><?php echo $row->informal_po_status;?></td>
